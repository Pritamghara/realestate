 ...
 const [listingupdated, setlistingupdated] = useState(false)
    ...
    <main className='p-3 max-w-4xl mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex- flex-col sm:flex-row gap-4'>
        <div className='flex flex-col gap-4 flex-1'>
          ...
          <textarea className='border p-3 rounded-lg' type="text" placeholder='description' maxLength={1500} id='description' required onChange={handleChange} value={formdata.description} />
          <input className='border p-3 rounded-lg' type="text" placeholder='address' id='address' maxLength='62' minLength={2} required onChange={handleChange} value={formdata.address} />
  ...
  formdata.imageUrls.length > 0 && formdata.imageUrls.map((url, index) => (
      <div key={url} className='flex justify-between p-3 border items-center'>
          ...
          <button disabled={loading  || uploading} className='mx-auto w-80 p-3 bg-slate-700 text-white rounded-lg uppercase hover:opacity-95 disabled:opacity-80'>{loading?"Creating...":"Create Listing"}</button>
